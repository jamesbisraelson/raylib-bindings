...
from typing import Union, Optional, Any, List, TypeVar, Callable, Type, cast
...
class Alias:
    ...
    def from_dict(obj: Any) -> 'Alias':
        ...
        return Alias(type, name, description, value)
...
class Header:
    ...
    def from_dict(obj: Any) -> 'Header':
        assert isinstance(obj, dict)
        defines = from_list(Alias.from_dict, obj.get("defines"))
        ...
        functions = from_list(Callback.from_dict, obj.get("functions"))
        return Header(defines, structs, aliases, enums, callbacks, functions)

    def to_dict(self) -> dict:
        result: dict = {}
        result["defines"] = from_list(lambda x: to_class(Alias, x), self.defines)
        ...
        result["functions"] = from_list(lambda x: to_class(Callback, x), self.functions)
        return result
